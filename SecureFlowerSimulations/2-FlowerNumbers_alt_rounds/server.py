...
class CustomFedAvg(FedAvg):
    ...
    def __init__(self, **kwargs):
        ...
        self.v_client_matrix = None 
        self.h_client_matrices = {}
    ...
    def configure_fit(self, server_round: int, parameters: Parameters, client_manager: ClientManager):
        ...
        # Initialize V-client matrix (3x3)
        if self.v_client_matrix is None:
            self.v_client_matrix = np.zeros((3, 3), dtype=np.float32)
        ...
        for client in clients:
            ...
            # In odd-numbered rounds, only 'client_V' receives a non-zero matrix
            if server_round % 2 == 0:
                # Check if the current client is the special client 'client_V'
                if client_name == "client_V":
                    # Send a customized matrix (twice the previous average) to 'client_V'
                    # This allows 'client_V' to carry over learning or specific information
                    custom_avg = self.previous_avg
                else:
                    # All other clients receive a zero matrix during odd-numbered rounds
                    # This means they won't get meaningful updates in this round
                    custom_avg = np.zeros((3, 3), dtype=np.float32)
            else:
                # In even-numbered rounds, all clients receive the same matrix (v_client_matrix)
                # This likely represents an aggregated view or a shared update (from V-client)
                custom_avg = self.v_client_matrix

            # Display which matrix is being sent to which client, useful for debugging or logging
            print(f"[Round {server_round}] Sending matrix to {client_name}:")
            print(custom_avg)
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        for client_proxy, res in results:
            ...
            if "client_name" in metrics:
                ...
                print(f"\033[94mReceived result from: {metrics['client_name']} {matrix}\033[0m")
            ...
            if client_name == "client_V":
                # Save matrix from V-client for use in next round
                self.v_client_matrix = matrix
            else:
                # Save matrix from H-clients (only non-V clients)
                self.h_client_matrices[client_name] = matrix
