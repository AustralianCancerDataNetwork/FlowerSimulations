        ...
        for client in clients:
            ...
            if server_round > 4:
                if server_round % 2 == 0:
                    ...
                    # The server has to send the parts of dZ1 to the H-clients,
                    # Specifically, based on their number of data points,
                    # and, also, based on their order of data points
                    # For V-client, we dont pay much concern
    
                    if client_name == "client_V":
                        custom_avg = self.dZ1
                    elif client_name == "client_H_1":
                        custom_avg = self.dZ1[0:200016]
                    elif client_name == "client_H_2":
                        custom_avg = self.dZ1[200016:4901168]
    ...
    def aggregate_fit(self, server_round, results, failures):
        ...
        if server_round > 4:
            if server_round % 2 == 1:
                ...
                # Concatenate the matrices received from the H-clients in order
                # As the client_H_1 holds the data for first 400 data points,
                # therefore, it will be placed first, followed by
                # the matrix of client_H_2 
                con_mat_h_clients = np.hstack((self.h_client_Zs["client_H_1"], self.h_client_Zs["client_H_2"]))

                # Add the concatenated matrix of H-clients with the
                # matrix of client_V 
                all_Z_matrices = con_mat_h_clients + self.v_client_Z
...
config = ServerConfig(num_rounds=2000)
