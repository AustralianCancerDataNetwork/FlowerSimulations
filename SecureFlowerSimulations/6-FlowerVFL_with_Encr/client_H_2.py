...
mask_client_H_2 = pd.read_csv(f'masks/{folder}/mask_client_H_2.csv')

# Convert the DataFrame to a NumPy array (matrix)
mask_client_H_2 = mask_client_H_2.to_numpy()
    ...
    def get_parameters(self, config):
        ...
        if self.current_round > 4:
            if self.current_round % 2 == 1:
                matrix_to_send = np.dot(self.W1, X_train.T) + mask_client_H_2
        ...
        if parameters:
            ...
            if self.current_round > 6:
                if self.current_round % 2 == 1:
                    ...
                    self.dZ1 = np.frombuffer(decrypted_bytes, dtype=np.float32).reshape(mask_client_H_2.shape)
