    ...
    def get_parameters(self, config):
        ...
        if self.current_round > 4:
            if self.current_round % 2 == 1:
                ...
                self.dZ1 = self.dZ1.astype(np.float32)
                dZ1_bytes = self.dZ1.tobytes()

                # Step 2: Pad the bytes (AES block size = 16 bytes)
                dZ1_padded_bytes = pad(dZ1_bytes, AES.block_size)
                ...
                dZ1_encrypted = cipher.encrypt(dZ1_padded_bytes)

                matrix_to_send = np.frombuffer(dZ1_encrypted, dtype=np.uint8)
